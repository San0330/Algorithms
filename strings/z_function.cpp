...
vector<int> compute_z2(string s) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        if (i <= right) {
            // corresponding ith (i-left)th element on prefix gives the matched
            // value but at position i, its value could to too large so count
            // from index i to rightmost index of window can be used whichever
            // is minimum
